...
## Installation
1. Run `git clone IAB-Net`.
2. Install dependencies by `pip install -r requirements.txt` (if necessary).
3. To install the cython-based evaluation toolbox, `cd` to `torchreid/eval_cylib` and do `make`. As a result, `eval_metrics_cy.so` is generated under the same folder. Run `python test_cython.py` to test if the toolbox is installed successfully. 
...
Please refer to [torchreid/models/\_\_init__.py](torchreid/models/__init__.py) for the keys to build these models.
...
--max-epoch 20 \ # maximum epoch to run
